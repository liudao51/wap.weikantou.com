...
class Apikit
{
     ...
     * @param bool $is_return_array 是否返回数据格式(默认false)
     *
     * @return object|array|null
     */
    public static function api_post($action, $data = array(), $header = array(), $timeout = 30, $is_return_array = false)
    {
        if (!Toolkit::is_string($action) || !Toolkit::is_array($data) || !Toolkit::is_array($header) || !Toolkit::is_integer($timeout)) {
            return null;
        }

        return self::curl_post($action, $data, $header, $timeout, 0, $is_return_array);
    }
     ...
     * @param bool $is_return_array 是否返回数据格式(默认false)
     *
     * @param bool $is_return_array
     */
    public static function api_get($action, $header = array(), $timeout = 30, $is_return_array = false)
    {
        //.....
    }
     ...
     * @return object|array|null
     */
    public static function api_post_ssl($action, $data = array(), $header = array(), $timeout = 30, $is_return_array = false)
    {
        if (!Toolkit::is_string($action) || !Toolkit::is_array($data) || !Toolkit::is_array($header) || !Toolkit::is_integer($timeout)) {
            return null;
        }

        return self::curl_post($action, $data, $header, $timeout, 1, $is_return_array);
    }
     ...
     * @return object|array|null
     */
    private static function curl_post($action, $data = array(), $header = array(), $timeout = 30, $protocol = 0, $is_return_array = false)
    {
        ...
        if (!isset($header) || !is_array($header) || count($header) == 0) {
            $header = array('Content-Type: application/json', 'X-Forwarded-For:' . Toolkit::get_client_ip());  //默认以json格式请求
        }

        if (in_array('Content-Type: application/json', $header)) {
            curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($data)); //把数据jsonencode后，注入参数
        } else {
            curl_setopt($curl, CURLOPT_POSTFIELDS, http_build_query($data));  //把数据urlencode后，注入参数
        }

        curl_setopt($curl, CURLOPT_HTTPHEADER, $header);  //设置请求头部
        ...
        $result_data = json_decode($http_response, $is_return_array);
